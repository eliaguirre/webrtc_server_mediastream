...
function SignalingChannel(sessionId) {
    ...
    var es = new EventSource("http://" + location.host + "/stoc/" + sessionId + "/" + userId);
    ...
    es.addEventListener("join", function (evt) {
        ...
        fireEvent({"type": "peer", "peer": channel,"user":peerUserId}, listeners);
    ...
    function PeerChannel(peerUserId) {
        ...
        function processSendQueue() {
            ...
            xhr.open("POST", "http://" + location.host + "/ctos/" + sessionId + "/" + userId + "/" + peerUserId);
...
var Peers = (function () {
    ...
    Peers.prototype.onAdd = function () {

    }

    Peers.prototype.add = function (peer) {
        this.enlaces.push(peer);
        this.onAdd();
    }

    Peers.prototype.send = function (msj) {
        for (var i in this.enlaces) {
            var p = this.enlaces[i];
            p.send(msj);
        }
    }
